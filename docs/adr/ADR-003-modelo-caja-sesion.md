...
5. **Venta que llega durante/después del cierre** → **RATIFICADO 2026-06-18: modo DEGRADADO, sin reapertura.** El cierre implementado es monolítico atómico (no hay ventana `CLOSING` observable), así que con `SELECT FOR UPDATE` la venta o se incluye en el arqueo (tomó el lock antes del cierre) o procede con `cash_register_session_id = NULL` (lock después) — **nunca se pierde**. Una vez cerrada, NO hay vuelta atrás: una venta nueva exige **abrir una caja nueva** (el POS nudgea al recibir la venta con sesión NULL). Se descartó la reapertura porque sobre un cierre atómico implicaría deshacer una conciliación de dinero ya persistida (rompe inmutabilidad del snapshot + complica auditoría BCRA). Si en operación real hiciera falta, se haría como épica de cierre-en-dos-fases, no como parche.

   **Múltiples sesiones por terminal por día (cambios de turno / breaks):** soportado nativamente. El índice único parcial `UNIQUE(tenant_id, point_of_sale_id) WHERE status='open'` solo restringe la caja abierta; al cerrar, la terminal queda libre para abrir una sesión nueva. Cada turno es una sesión con su propio `opening_amount`, arqueo y `opened_by`/`closed_by` (rendición por turno).
