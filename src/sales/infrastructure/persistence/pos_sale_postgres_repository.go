...
// Create persiste una nueva venta POS con sus items (atomically). Parent (pos_sales) e
// hijos (pos_sale_items) se insertan dentro de la MISMA transacción RLS (T5): la policy
// tenant_isolation de pos_sale_items evalúa por subquery contra pos_sales, y solo ve el
// parent recién insertado si comparte contexto/tx — mismo requisito que Save en
// order_postgres_repository.go (T4, @dev-architect).
...
func (r *PosSalePostgresRepository) Create(ctx context.Context, sale *entity.PosSale) error {
	rc := postgres.RLSContext{TenantID: sale.TenantID.String()}
...
func (r *PosSalePostgresRepository) GetByID(ctx context.Context, tenantID, saleID uuid.UUID) (*entity.PosSale, error) {
	...
	err := postgres.WithRLSInTransaction(ctx, r.db, rc, func(ctx context.Context, tx *sql.Tx) error {
		...
		err := tx.QueryRowContext(ctx, querySale, saleID, tenantID).Scan(
			&sale.ID,
			&sale.TenantID,
			&sale.CustomerID,
			&sale.PaymentMethodID,
		...
		if err == sql.ErrNoRows {
			return entity.ErrPosSaleNotFound
		}
		if err != nil {
			return fmt.Errorf("error querying pos_sale: %w", err)
		}
		...
		queryItems := `
			...
			ORDER BY created_at
		`
		rows, err := tx.QueryContext(ctx, queryItems, sale.ID)
		if err != nil {
			return fmt.Errorf("error querying pos_sale_items: %w", err)
		}
		defer rows.Close()
		...
		for rows.Next() {
			...
			if err := rows.Scan(
				...
				return fmt.Errorf("error scanning pos_sale_item: %w", err)
			}
			items = append(items, item)
		}
		if err := rows.Err(); err != nil {
			return fmt.Errorf("error iterating pos_sale_items: %w", err)
		}
		sale.Items = items
...
func (r *PosSalePostgresRepository) ListByTenant(ctx context.Context, tenantID uuid.UUID) ([]*entity.PosSale, error) {
	...
	err := postgres.WithRLSInTransaction(ctx, r.db, rc, func(ctx context.Context, tx *sql.Tx) error {
		...
		rows, err := tx.QueryContext(ctx, querySales, tenantID)
		if err != nil {
		...
		for rows.Next() {
			...
			if err != nil {
				rows.Close()
				return fmt.Errorf("error scanning pos_sale: %w", err)
			}
		...
		if err := rows.Err(); err != nil {
			rows.Close()
			return fmt.Errorf("error iterating pos_sales: %w", err)
		}
		// rows debe cerrarse ANTES de abrir el siguiente QueryContext: dentro de
		// WithRLSInTransaction todo corre sobre el mismo *sql.Tx (una sola conexión vía
		// lib/pq), que no soporta dos result sets abiertos a la vez (mismo hallazgo que
		// order_postgres_repository.go List, T4).
		rows.Close()
		...
		for _, sale := range sales {
			itemRows, err := tx.QueryContext(ctx, queryItems, sale.ID)
			if err != nil {
				return fmt.Errorf("error querying pos_sale_items: %w", err)
			}
...
func (r *PosSalePostgresRepository) CountByTenant(ctx context.Context, tenantID uuid.UUID) (int, error) {
	...
	err := postgres.WithRLSInTransaction(ctx, r.db, rc, func(ctx context.Context, tx *sql.Tx) error {
		...
		if err := tx.QueryRowContext(ctx, query, tenantID).Scan(&count); err != nil {
			return fmt.Errorf("error counting pos_sales: %w", err)
		}
		return nil
	})
	if err != nil {
...
func (r *PosSalePostgresRepository) CountSalesTodayByTenant(ctx context.Context, tenantID uuid.UUID) (int, error) {
	...
	err := postgres.WithRLSInTransaction(ctx, r.db, rc, func(ctx context.Context, tx *sql.Tx) error {
		...
		`
		if err := tx.QueryRowContext(ctx, query, tenantID).Scan(&count); err != nil {
			return fmt.Errorf("error counting pos_sales today: %w", err)
		}
		return nil
	})
	if err != nil {
