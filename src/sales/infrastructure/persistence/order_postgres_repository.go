...
// Parent (sales_orders) e hijos (sales_order_items) se insertan dentro de la MISMA
// transacción RLS: la policy tenant_isolation de sales_order_items evalúa por subquery
// contra sales_orders, y solo ve el parent recién insertado si comparte contexto/tx (T2).
func (r *OrderPostgresRepository) Save(ctx context.Context, order *entity.Order) error {
	rc := postgres.RLSContext{TenantID: order.TenantID}
...
func (r *OrderPostgresRepository) FindByID(ctx context.Context, orderID, tenantID string) (*entity.Order, error) {
	...
	err := postgres.WithRLSInTransaction(ctx, r.db, rc, func(ctx context.Context, tx *sql.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("error finding order items: %w", err)
		}
		defer rows.Close()
		...
		for rows.Next() {
			...
			if err != nil {
				return fmt.Errorf("error scanning order item: %w", err)
			}
...
func (r *OrderPostgresRepository) UpdateOrderNumber(ctx context.Context, orderID, tenantID string, orderNumber int) error {
	...
	return postgres.WithRLSInTransaction(ctx, r.db, rc, func(ctx context.Context, tx *sql.Tx) error {
		...
		_, err := tx.ExecContext(ctx, query, orderNumber, orderID, tenantID)
		if err != nil {
...
func (r *OrderPostgresRepository) GetOrderFinancialSummary(ctx context.Context, orderID, tenantID string) (*entity.OrderFinancialSummary, error) {
	...
	err := postgres.WithRLSInTransaction(ctx, r.db, rc, func(ctx context.Context, tx *sql.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("error getting order financial summary: %w", err)
		}
		if lastAt.Valid {
...
func (r *OrderPostgresRepository) List(ctx context.Context, tenantID string, page, pageSize int, statusFilter string) ([]*entity.Order, int, error) {
	...
	err := postgres.WithRLSInTransaction(ctx, r.db, rc, func(ctx context.Context, tx *sql.Tx) error {
		...
		if err := tx.QueryRowContext(ctx, queryCount, countArgs...).Scan(&totalCount); err != nil {
			return fmt.Errorf("error counting orders: %w", err)
		}
		...
		rows, err := tx.QueryContext(ctx, queryOrders, queryArgs...)
		if err != nil {
		...
		for rows.Next() {
			...
			if err != nil {
				rows.Close()
				return fmt.Errorf("error scanning order: %w", err)
			}
		...
		// 4. Cargar items de cada orden con snapshots — rows ya cerrado: tx (lib/pq) es una
		// sola conexión, no soporta un segundo QueryContext con el anterior todavía abierto.
		queryItems := `
			SELECT id, sales_order_id, sku, quantity, unit_price, product_snapshot, variant_snapshot
			FROM sales_order_items
			WHERE sales_order_id = $1
		...
		for _, order := range orders {
			itemRows, err := tx.QueryContext(ctx, queryItems, order.OrderID)
			if err != nil {
				return fmt.Errorf("error loading items for order %s: %w", order.OrderID, err)
			}
